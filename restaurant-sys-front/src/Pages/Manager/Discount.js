  ...
  const fetchFoods = async () => {
    try {
      const res = await axios.get(
        "http://127.0.0.1:8000/api/Manager/GetFoodDiscount",
        {
      ...
      toast.error("Failed to load discounted foods");
    }
  };
    ...
    <div className="p-6" style={{ paddingLeft: "20px", width: "100vw", paddingRight: "10px" }}>
      ...
      <div className="" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h1 style={{ color: "orange", marginBottom: "15px", fontSize: "28px" }}>
          <i className="fas fa-percent" style={{ marginRight: "10px" }}></i>
          Discounted Items
        </h1>
        {/* لا يوجد زر إضافة هنا */}
      </div>
